"""Audit original full-phase debugger views using retained study 11 recordings.
...
def run(root, data, output, *, view_path=None):
    ...
    report = {'status': 'online_view_projection_audited', 'chunk': summary['chunk'],
        ...
        'source_sha256': {name: digest(ROOT / name) for name in (
            'paperlab/fly_online_projection.py', 'paperlab/fly_view_projection.py',
            'paperlab/fly_online_audit.py')},
