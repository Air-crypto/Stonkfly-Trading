"""Independently reconstruct stimulation results from full native recordings."""
import base64
import io
import json
...
from .fly_paper_stimulation import INFERENCE, SOURCE, TARGETS, sequences, validate
...
def audit(study, payload, artifacts):
    p, ref, plan = validate(payload); root = Path(artifacts); seq = sequences(payload)
    require(study['status'] == 'paper_stimulation_completed' and study['protocol'] == p, 'Study protocol differs')
    require(list(study['reports']) == list(p['arms']), 'Missing or reordered stimulation conditions')
    require(study['controls_verified'] == [n for n,a in p['arms'].items() if not a['current']], 'Missing controls')
    require(study['news_audit'] == audit_news(payload['paper_plan'], root/'news.db'), 'News audit differs')
    ...
    lookup = {str(v): i for i,v in enumerate(ids)}; targets = [lookup[x] for x in TARGETS]
    ...
    for name, arm in p['arms'].items():
        ...
        require(report['source'] == SOURCE and report['native_build'] == ref['native_build'] and report['plan_sha256'] == p['plan_sha256'] and report['reference_report_sha256'] == p['reference_report_sha256'], 'Recording provenance differs')
        ...
        for j,(event,(rgb,row),f) in enumerate(zip(events,seq[arm['pool']],view['frames']),1):
            ...
            require(event['stimulation'] == {'target_ids':TARGETS,'current':arm['current'],'duration_ms':500,'artifact_sha256':digest(current_path)}, 'Current provenance differs')
            require(set(currents) == {'durations_ms','currents','target_indices'} and np.array_equal(currents['target_indices'],targets), 'Applied current targeted different cells')
            require(np.array_equal(currents['durations_ms'],np.full(50,10)) and np.array_equal(currents['currents'],np.full((50,2),arm['current'])), 'Applied current or duration differs')
            ...
            if not arm['current']:
                expected = ref['phase_diagnostics'][arm['pool']][arm['memory']+'_frozen']['test']['decisions'][j-1]['neural']
                require(matches(event,expected), 'Original zero-current control did not reproduce')
    ...
    for current in p['currents']:
        for i,key in enumerate(plan['registration']['cohort']):
            a=f'pool{i}-pristine-current{current}'; b=f'pool{i}-trained-current{current}'
            comparisons.append({'pool':key,'current':current,'pristine':a,'trained':b,
    ...
    return {'status':'verified','protocol':p,'full_observations_verified':36,'native_bins_verified':1800,
            ...
            'artifact_sha256':hashes,'interpretation':p['interpretation']}
