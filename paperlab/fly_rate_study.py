...
from .fly_rate_protocol import ARMS, HOSTING_ALLOCATION, chunk_name, chunk_order, development_choice
...
from .fly_rate_metrics import trading_metrics, replay_ledger

SOURCE_FILES = tuple(dict.fromkeys((*PAPER_SOURCES, 'fly_rate_protocol.py', 'fly_rate_study.py',
    'fly_rate_audit.py', 'fly_rate_inputs.py', 'fly_rate_metrics.py',
    'fly_learning_scale_credit.py', 'fly_rate_checkpoint.py', 'fly_credit_audit.py', 'fly_credit_reset_audit.py', 'fly_trace_memory.py',
    'fly_rate_price_audit.py')))


def select_development(envelope, chunks):
    p = validate(envelope)
    if p['registration']['study'] != '12':
        raise ValueError('Expected study 12 inputs')
    ...
    provenance = {}; timelines = {arm: [] for arm in ARMS}
    for stage, pool, arm in chunk_order()[:8]:
        ...
        if (result.get('status') != 'paper_rate_chunk_completed' or result.get('chunk') != name
                ...
                or result.get('pool') != p['registration']['cohort'][pool]):
        ...
        ledger=replay_ledger(result['outcome'],p['series'][result['pool']],
                             p['registration']['development_start'],ARMS[arm])
        if result.get('trading_metrics')!=ledger['metrics']:
            raise ValueError('Development metrics differ from replayed ledger')
        equities[arm] += ledger['equity']
        counts[arm].append(len(ledger['coverage']));timelines[arm].append(ledger['coverage'])
        provenance[name] = signature(result)
    if any(t!=timelines['pristine_frozen'] for t in timelines.values()):
        raise ValueError('Development arms have different decision timestamps')
    return {'plan_sha256': envelope['sha256'], 'selected': development_choice(equities, counts),
            ...
            'development_chunk_sha256': provenance, 'development_timestamps':timelines,
            'ledgers_replayed_before_selection':True,'test_simulated_before_selection': False}
...
def run_chunk(envelope, memory_root, news_archive, data, output, *, stage, pool_index, arm,
    ...
    if r['study'] != '12':raise ValueError('Online chunk runner requires study 12')
    ...
    if (root/'trace/view.json').exists():
        ...
        view['report'].update(evaluation_phase=stage, memory_origin=declared['memory'],
            plan_sha256=envelope['sha256'], study='12', pool=key, chunk=name,
    ...
    result = {'status': 'paper_rate_chunk_completed', 'chunk': name, 'stage': stage,
        'pool_index': pool_index, 'pool': key, 'arm': arm, 'plan_sha256': envelope['sha256'],
        'registration': r, 'selection': selection, 'outcome': outcome,
        'trading_metrics': trading_metrics(outcome['rows']), 'native_build': b.build,
    ...
    print(json.dumps({'event': 'paper_rate_chunk_completed', 'chunk': name,
...
def main():
    ...
    run_chunk(json.loads(args.plan.read_text()), args.memory, args.news, args.fly_data, args.out,
        ...
        development=json.loads(args.development.read_text()) if args.development else None,seconds=args.seconds)
