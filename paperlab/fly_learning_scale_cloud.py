"""Prepared bridge for the post-study learning-rate assay; not deployed during study 11.

The unscheduled learning-rate worker shares the existing worker lease and budget.
...
from .fly_learning_scale import source_hashes, validate, verify_reference, run as capture
from .fly_learning_scale_audit import audit
...
CLAIM = 'learning-scale-01-claim.json'
INPUTS = 'fly-scale-inputs'
...
def validate_request(request):
    if (not isinstance(request, dict) or set(request) != {'run_id', 'scale_plan'}
            or not isinstance(request['run_id'], str)
            or not re.fullmatch(r'assay-scale-[a-z0-9-]{1,64}', request['run_id'])):
        raise ValueError('Expected a learning-rate assay request and safe run ID')
    plan = request['scale_plan']
    if not isinstance(plan, dict) or set(plan) != {
            'payload', 'bundle_sha256', 'study_report_sha256', 'source_sha256'}:
        raise ValueError('Expected the exact learning-rate request fields')
...
def check_bundle(path, expected_bundle, expected_report):
    with tempfile.TemporaryDirectory(prefix='scale-release-') as temporary:
...
def run_budgeted_request(request, state, data, *, call_id, input_id, commit):
    ...
    state = Path(state); root = state/'fly-debugger'; plan = request['scale_plan']
    ...
    if (root/CLAIM).exists() or (root/request['run_id']).exists():
        raise ValueError('Learning-rate assay already claimed; inspect its existing call')
    ...
    reservation = reserve(state/'budget.json', True, seconds=1080,
                          startup_seconds=30, memory_gib=8, limit_override=25)
    if reservation is None:
        return {'status': 'budget_stopped'}
    # 370 seconds at 3x provider price, retaining the budget module's 2x margin.
    reservation['rate'] *= 3
...
def run_request(request, root, data, *, call_id, input_id, commit):
    """Worker adapter: release validation, durable claim, then one capture."""
    protocol, parent = validate_request(request)
    root = Path(root); plan = request['scale_plan']; output = root/request['run_id']
    ...
    if claim_path.exists() or output.exists():
        raise ValueError('Learning-rate assay already claimed; inspect the original call, never restart')
    with tempfile.TemporaryDirectory(prefix='scale-release-') as temporary:
        ...
        try:
            ...
            result = {'status': 'learning_scale_completed', 'run_id': request['run_id'],
...
def terminal_result(result, request, receipt):
    ...
    if (result.get('status') != 'learning_scale_completed'
            ...
            or report.get('status') != 'learning_scale_captured_pending_audit'
            or report.get('protocol') != protocol
            or report.get('code_sha256') != request['scale_plan']['source_sha256']
            or report.get('completed_study_sha256') != request['scale_plan']['study_report_sha256']
            ...
            or not 0 < result['budget'].get('reserved_usd', 0) <= .0975912):
...
def idle_worker(modal):
    ...
    if owner is not None or any(s.num_total_runners or s.num_running_inputs or s.backlog for s in stats):
        raise RuntimeError('Worker has active work; no learning-rate assay submitted')
    ...
    if existing is not None:
        raise RuntimeError('Learning-rate assay already claimed remotely; inspect the original call')
...
def cloud_run(payload, output, reference, data, *, completed_study):
    ...
    if not receipt_path.exists():
        ...
        with tempfile.TemporaryDirectory(prefix='scale-prepare-') as temporary:
            ...
            request = {'run_id': 'assay-scale-'+uuid.uuid4().hex, 'scale_plan': {
    ...
    if (receipt['request_sha256'] != signature(request)
            or request['scale_plan']['payload'] != payload or receipt['run_id'] != request['run_id']):
    ...
    with tempfile.TemporaryDirectory(prefix='scale-audit-') as temporary:
        study = Path(temporary)/'study'
