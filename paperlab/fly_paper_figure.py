...
def evidence(report):
    ...
    if (report.get('status') != 'paper_checkpoint_study_completed'
            ...
            or r['arms'] != ARMS or set(report['total_equity']) != set(ARMS)
    ...
    # Matched comparisons: reset scope is the same on both sides of each subtraction.
    effects = {phase: {
        'carry': report['total_equity']['trained_frozen'][phase] - report['total_equity']['pristine_frozen'][phase],
        'reset': report['total_equity']['trained_input_reset'][phase] - report['total_equity']['pristine_input_reset'][phase],
    } for phase in PHASES}
...
def render(report, output, *, fixture=False):
    ...
    with plt.rc_context({'font.size': 11, 'font.family': 'DejaVu Sans',
        ...
        title = 'Paper-trained connection memory: does it help?'
        if fixture:
        ...
        fig.text(.035, .918, 'Each phase starts with $1,000 and fresh activity. Only connection weights/u/w are imported.', color=muted)
        ...
        for j, phase in enumerate(PHASES):
            ...
            for arm in ARMS:
                ...
                line, = ax.plot(xs, ys, color=colors[arm], lw=2,
                                linestyle='--' if 'input_reset' in arm else '-',
                                marker='s' if 'input_reset' in arm else 'o', markersize=4,
                                label=LABELS[arm])
                if not j:
            ...
            for (row, col), cell in table.get_celld().items():
                ...
                if row and col == 0:
                    cell.set_text_props(color=colors[list(ARMS)[row - 1]])
        ...
        for j, phase in enumerate(PHASES):
            for k, (reset, color) in enumerate((('carry', '#67e8cf'), ('reset', '#e6a8e8'))):
                value = effects[phase][reset];x = j + (k - .5) * .3
                ax.bar(x, value, width=.25, color=color,
                       label=('Carry input state' if reset == 'carry' else 'Reset input state') if j == 0 else None)
