...
function chart(id, xs, series, unit, cursor){
 ...
 for(let k=0;k<3;k++){const v=ymin+(ymax-ymin)*k/2;add('text',{x:left-8,y:Y(v)+4,'text-anchor':'end',fill:'#b2bfd1','font-size':11},num(v,2));}
 ...
 add('text',{x:left,y:12,fill:'#b2bfd1','font-size':11},unit);add('text',{x:(left+right)/2,y:173,'text-anchor':'middle',fill:'#b2bfd1','font-size':11},'Neural time (ms)');
...
function network(f){
 ...
 for(const e of data.edges){const [x,y]=positions[e.source],[tx,ty]=positions[e.target],pi=pmap.get(e.plastic_index),changed=pi!==undefined&&Math.abs(f.plastic_weights[bin][pi]-f.plastic_initial[pi])>1e-9,active=f.counts[bin][e.source]>0;
 add('path',{d:`M${x},${y} Q${(x+tx)/2+12},${(y+ty)/2-8} ${tx},${ty}`,fill:'none',stroke:changed?'#ffbf69':active?'#67e8cf':'#35445c',opacity:changed?.65:active?.3:.2,'stroke-width':changed?1.1:.6});}
...
function draw(){if(!data)return;const f=data.frames[step],n=data.nodes[node];bin=Math.min(bin,f.times_ms.length-1);$('bin').max=f.times_ms.length-1;$('bin').value=bin;$('time').textContent=num(f.times_ms[bin])+' ms';network(f);
 ...
 $('input').src='data:image/png;base64,'+f.input_png;$('input-info').textContent=`${data.report.config.preset} · ${data.report.config.view} · ${data.report.config.news} news`;
 $('decision').textContent=f.event.side;dl($('decoder'),[['Left DNp20 (Hz)',f.event.left_hz],['Right DNp20 (Hz)',f.event.right_hz],['Right − left (Hz)',f.event.difference_hz],['Gate spikes',f.event.gate_spikes],['Reinforcement',f.event.stimulus]]);
 dl($('metrics'),[['Plasticity enabled',f.event.diagnostics.plasticity_enabled??data.report.config.learning],['Changed edges at cursor',f.changed_edges[bin]],['Weight Δ L2 at cursor',f.weight_delta_l2[bin]],['Memory u L2',f.memory_u_l2[bin]],['Memory w L2',f.memory_w_l2[bin]],['Loss',null],['Backprop gradient',null],['Clipped edges (end)',f.event.diagnostics.clipped_edges]]);
 chart('spike-chart',f.times_ms,[{name:'All retained neurons',values:f.total_spikes,color:'#67e8cf'}],'Spikes / 10 ms',bin);
 chart('weight-chart',f.times_ms,[{name:'All plastic edges: L2 from observation start',values:f.weight_delta_l2,color:'#ffbf69'}],'Weight Δ L2',bin);
 ...
 const plastic=data.edges.filter(e=>e.plastic_index!==null),e=plastic[edge];if(e){const pi=data.plastic_selection.indexOf(e.plastic_index);$('edge-detail').textContent=`Edge ${e.id} · ${data.nodes[e.source].id} → ${data.nodes[e.target].id} · weight ${num(f.plastic_weights[bin][pi],6)} · Δ ${num(f.plastic_weights[bin][pi]-f.plastic_initial[pi],6)} · source spikes ${f.counts[bin][e.source]}`;chart('edge-chart',f.times_ms,[{name:'Synaptic weight',values:f.plastic_weights.map(v=>v[pi]),color:'#ffbf69'}],'Weight (model units)',bin);}
}
...
function load(d){if(!d.report||!d.nodes?.length||!d.frames?.length)throw new Error('Not a fly view.json recording');data=d;marketTimeline(d.report);const market=d.report.source==='sealed_retrospective_market_replay';$('source-badge').textContent=market?'Market replay · simulated fills':'Synthetic assay · paper only';$('decoder-note').textContent=market?'Whole-observation result (500 ms). Simulated fills are recorded in the study ledger. This viewer sends no orders.':'Whole-observation result (500 ms). BUY/SELL are engineered mappings of downstream firing rates; this assay executes no trades.';step=bin=node=edge=0;clearInterval(timer);timer=null;$('play').textContent='Play bins';options($('step'),d.frames.map((f,i)=>[i,`${i+1} · ${f.event.side}`]));options($('node'),d.nodes.map((n,i)=>[i,`${n.group}: ${n.type||n.id} · ${n.id}`]));options($('edge'),d.edges.filter(e=>e.plastic_index!==null).map((e,i)=>[i,`${e.id} · ${d.nodes[e.source].type} → ${d.nodes[e.target].type}`]));$('graph-size').textContent=`${num(d.report.graph.neurons)} neurons / ${num(d.report.graph.edges)} connections simulated`;$('selection').textContent=`Showing ${d.nodes.length} neurons and ${d.edges.length} connections. ${d.selection}`;$('provenance').textContent=JSON.stringify(d.report,null,2);$('status').textContent=`Loaded ${runName||'imported trace'} · ${d.frames.length} observations · ${num(d.report.seconds,1)} s capture time`;$('comparison').replaceChildren();$('compare').value='';draw();}
...
async function refresh(){const r=await api('/api/runs');const prior=$('runs').value;options($('runs'),r.runs.map(n=>[n,n]));options($('compare'),[['','Choose a run'],...r.runs.map(n=>[n,n])]);$('run').disabled=!r.can_run;$('run').textContent=!r.can_run?'Read-only recordings':r.backend==='modal'?'Run on Modal (cloud)':'Run full-network assay';if(r.runs.includes(prior))$('runs').value=prior;if(!data&&r.runs.length)await loadRun(r.runs.at(-1));if(!r.runs.length&&!data)$('status').textContent='No recordings yet. Run an assay or import a view.json.';}
...
$('assay').onsubmit=async e=>{e.preventDefault();$('error').textContent='';const f=new FormData(e.target);const config=Object.fromEntries(f);config.steps=Number(f.get('steps'));config.eta=Number(f.get('eta'));config.current=Number(f.get('current'));config.learning=f.has('learning');config.reinforcement_only=f.has('reinforcement_only');config.neurons=String(f.get('neurons')).split(',').map(x=>x.trim()).filter(Boolean);try{$('run').disabled=true;await api('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(config)});await status();}catch(e){$('run').disabled=false;error(e);}};
...
$('compare').onchange=async e=>{try{
 ...
 const current=data,request=++viewRequest,other=await api('/api/view?run='+encodeURIComponent(e.target.value));
 if(request!==viewRequest||current!==data)return;
 const market=current.report.source==='sealed_retrospective_market_replay',otherMarket=other.report.source==='sealed_retrospective_market_replay';
...
window.addEventListener('resize',()=>draw());refresh().then(()=>status(true)).catch(error);
