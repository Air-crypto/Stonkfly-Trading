...
function draw(){if(!data)return;externalNeuron=false;document.querySelector('.legend .changed').parentElement.lastChild.textContent=$('pristine').checked?'Plastic edge differs from pristine state':'Plastic edge changed since observation start';const f=data.frames[step],n=data.nodes[node];bin=Math.min(bin,f.times_ms.length-1);$('bin').max=f.times_ms.length-1;$('bin').value=bin;$('time').textContent=num(f.times_ms[bin])+' ms';network(f);decoderPath(f);activityBoundary(f);
 ...
 $('phase-note').textContent=f.event.phase==='probe'?`Frozen probe: neural activity was reset; ${data.report.probe_boundary.restoration?.mode&&data.report.probe_boundary.restoration.mode!=='none'?`${num(data.report.probe_boundary.restoration.edge_count)} connections restored to pristine memory (${data.report.probe_boundary.restoration.mode})`:'trained weights were retained'}. Weight L2 from pristine: ${num(data.report.probe_boundary.weight_delta_from_pristine_l2,6)}. No reinforcement or extra current.`:data.report.source==='matched_market_stimulus_assay'?`Historical replay: ${data.report.config.memory} memory; ${data.report.config.learning?'updates enabled':'weights and memory frozen'}; pulses ${data.report.config.pulses}.${data.report.restoration?.post_ids?.length?` Restored ${num(data.report.restoration.edge_count)} incoming connections to body IDs ${data.report.restoration.post_ids.join(', ')} before replay.`:''} No fills or returns recomputed.`:data.report.restoration?.post_ids?.length?`Market replay: ${num(data.report.restoration.edge_count)} incoming connections to body IDs ${data.report.restoration.post_ids.join(', ')} restored to pristine before inference. Training memory is retained elsewhere; inference is frozen.`:'';$('decision').textContent=f.event.side;dl($('decoder'),[['Left DNp20 (Hz)',f.event.left_hz],['Right DNp20 (Hz)',f.event.right_hz],['Right − left (Hz)',f.event.difference_hz],['Gate spikes',f.event.gate_spikes],['Reinforcement',f.event.stimulus]]);
 if(data.report.memory_origin){
  const exposure=data.report.training_exposure;
  $('phase-note').textContent=`${data.report.evaluation_phase==='development'?'Development':'Test'} evaluation: ${data.report.memory_origin==='paper_trained'?'paper-trained':'pristine'} connection memory, frozen throughout. Fresh neural activity at phase start; ${data.report.config.activity_reset==='conductance'?'synaptic input state g reset before each later image':'activity carried between images'}.${exposure?` Training exposure: ${num(exposure.observations)} observations; ${num(exposure.positive_rewards)} positive and ${num(exposure.negative_rewards)} negative paper rewards.`:''} Simulated fills; this does not establish profitable learning.`;
 }
