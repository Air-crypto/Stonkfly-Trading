...
The new experiment at `/state/fast-5m` preserves the original `/state` experiment. Closed-candle price context supplies the missing initial lookback, so paper decisions can begin before 64 fresh observations; see [historical warm-up](observability.md#historical-price-warm-up). The prefix disappears as fresh observations arrive. If fetching context initially fails, the runner continues normal collection until 64 fresh observations are available. Fly plasticity then updates from its own paper results; the compact bootstrap is used initially. After 400 snapshots, a fresh compact candidate is trained every six hours on the chronological training partition of the observed archive. No candidate is chosen by test profit. Current news is never backdated into the historical bootstrap.
