...
if not (FULL_FLY and UNIVERSE):
    raise ValueError('Selective assay requires PAPERLAB_FLY=1 and PAPERLAB_UNIVERSE=1')

ROOT = Path(__file__).parent
app = modal.App('fly-paper-selective-01')
...
for name in ('fly-first-drive-01.json', 'fly-market-study-11-preregistration.json'):
    image = image.add_local_file(ROOT/'reports'/name, '/opt/paperlab/reports/'+name, copy=True)


def _verify_import():
    import importlib
    importlib.import_module('selective_cloud')
    from paperlab.fly_selective_trace_cloud import run_budgeted_request
    from paperlab.fly_selective_trace import source_hashes
    source_hashes()  # Fail the build if any execution or audit dependency is missing.
    print('Selective assay worker import passed', flush=True)
