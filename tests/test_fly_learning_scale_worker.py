...
from test_fly_learning_scale_cloud import worker, request
...
def test_nonpreemptible_reservation_precedes_capture_and_failed_claim_cannot_repeat(worker, monkeypatch, fail):
    ...
    def capture(payload, reference, data, output, *, completed_study):
        ...
        assert json.loads(budget.read_text())['months'][month] == pytest.approx(.0975912)
    ...
    if fail:
        ...
        assert b['reserved_usd'] == pytest.approx(.0975912) and b['price_multiplier'] == 3
...
def test_unscheduled_worker_shares_lease_and_imports_in_packaged_layout(tmp_path):
    tree = ast.parse(Path('scale_cloud.py').read_text())
    ...
    for name, value in {'nonpreemptible': True, 'cpu': (2, 2), 'memory': (8192, 8192),
        'timeout': 360, 'retries': 0, 'max_containers': 1, 'min_containers': 0,
    ...
    for name in ('cloud.py', 'selective_cloud.py', 'scale_cloud.py'): (tmp_path/name).write_bytes(Path(name).read_bytes())
    result = subprocess.run([sys.executable, '-I', '-c',
        'import sys;sys.path.insert(0,sys.argv[1]);import scale_cloud;print("import passed")', str(tmp_path)],
