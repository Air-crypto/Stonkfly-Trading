...
from paperlab import fly_learning_scale_cloud as bridge
from paperlab.core import atomic_json, digest
from paperlab.fly_market_study import signature
from test_fly_selective_trace import payload as original_payload


def payload():
    return {"protocol_json":Path("reports/fly-learning-scale-protocol-01.json").read_text(),
            "selective_payload":original_payload()}
...
def request(release):
    return {'run_id': 'assay-scale-test', 'scale_plan': {
...
def test_actual_incomplete_study_cannot_reach_modal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('paperlab.fly_learning_scale.TraceLab', forbidden)
    out = tmp_path/'output'
...
def test_request_is_bound_to_protocol_sources_and_safe_identifiers(transport, change):
    ...
    if change == 'path': req['run_id'] = 'assay-scale-../../escape'
    elif change == 'extra': req['config'] = {}
    elif change == 'protocol': req['scale_plan']['payload']['protocol_json'] += ' '
    elif change == 'source': req['scale_plan']['source_sha256'] = {}
    else: req['scale_plan'][{'bundle': 'bundle_sha256', 'report': 'study_report_sha256'}[change]] = '../bad'
...
def cloud(transport, monkeypatch):
    ...
    def spawn(**kwargs):
        ...
        return SimpleNamespace(object_id='fc-scale-once')
    def get(timeout):
        assert 0 <= timeout <= 50
...
def test_timeout_reattaches_one_call_and_uses_saved_bundle(transport, cloud, tmp_path):
    ...
    assert cloud.observed == ['fc-scale-once']*3
...
def test_remote_failure_does_not_restart(transport, cloud, tmp_path):
    ...
    assert len(cloud.calls) == 1 and cloud.observed == ['fc-scale-once']*2


def fixture_result(req, volume):
    p, _ = bridge.validate_request(req)
    report = {'status': 'learning_scale_captured_pending_audit', 'protocol': p,
              'code_sha256': req['scale_plan']['source_sha256'],
              'completed_study_sha256': req['scale_plan']['study_report_sha256']}
    ...
    return {'status': 'learning_scale_completed', 'run_id': req['run_id'], 'remote_path': base,
            'report': report, 'artifact_sha256': hashes, 'request_sha256': signature(req),
            'call_id': 'fc-scale-once', 'input_id': 'in-scale-once',
            'budget': {'monthly_limit_usd': 25, 'estimated_compute_usd': 0.01, 'provider_bill': False,
                       'nonpreemptible': True, 'price_multiplier': 3, 'reserved_usd': .0975912}}
...
def test_interrupted_download_resumes_then_audits_before_installing_views(transport, cloud, tmp_path, monkeypatch):
    ...
    assert len(cloud.result['artifact_sha256']) == 77
    ...
    for arm in cloud.result['report']['protocol']['arms']:
        assert json.loads((root/arm/'remote.json').read_text())['call_id'] == 'fc-scale-once'
...
def test_worker_commits_claim_before_capture_and_refuses_a_second_run(worker):
    ...
    assert result['status'] == 'learning_scale_completed' and events == ['commit', 'capture', 'commit']
    assert len(result['artifact_sha256']) == 77
    req['run_id'] = 'assay-scale-different-output'
...
def test_worker_bad_release_does_not_claim_or_construct(worker):
    req, root, events, commit = worker
    req['scale_plan']['study_report_sha256'] = '0'*64
