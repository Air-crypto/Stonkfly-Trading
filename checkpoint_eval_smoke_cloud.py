...
def worker(run_id: str,all_observed: bool=False):
    return exclusive('worker',_smoke,run_id,all_observed)

def _smoke(run_id,all_observed=False):
    ...
    if all_observed:from paperlab.solana_universe import QUOTE_PROTOCOL
    ...
    try:
        ...
        created=dict(kind='CreateEvent',mint=mint,quote_mint=ZERO,token_program=TOKEN,is_mayhem_mode=all_observed,
                     received=t0-60,timestamp=int(t0-7200 if all_observed else t0-60),slot=1,signature='synthetic-create',log_index=0)
        db.execute('insert into events values(?,?,?)',(created['received'],0,json.dumps(created)))
        rows=[]
        for i in range(18):
            obs=t0+i*5;received=obs-.1
            trade=dict(kind='TradeEvent',mint=mint,quote_mint=ZERO,mayhem_mode=all_observed,received=received,timestamp=int(received),slot=2+i,
        ...
        plan=dict(protocol=PROTOCOL,quote_protocol=QUOTE_PROTOCOL,all_observed=all_observed,cutoff=t0-1,policies=['untrained',cp['id']],checkpoints=[cp],
