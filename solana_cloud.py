...
image=(base_image.pip_install('websockets==16.0')
...
def worker(run_id: str, seconds: int=900, probe: bool=False):
    return exclusive('worker',_run,run_id,seconds,probe)


def _run(run_id,seconds,probe):
    import json,re,time
    ...
    atomic_json(root/'owner.json',dict(call_id=modal.current_function_call_id(),input_id=modal.current_input_id(),
        run_id=run_id,probe=probe,at=time.time(),source_sha256={
            n:digest('/opt/paperlab/'+n) for n in ('solana_cloud.py','paperlab/solana_events.py',
                    'paperlab/solana_paper.py','paperlab/solana_schema/pump_events.json')},reservation=reservation))
    volume.commit()
    try:
        if probe:
