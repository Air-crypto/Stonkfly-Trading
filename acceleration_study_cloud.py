...
@app.function(image=image,volumes={'/state':volume},cpu=(2,2),memory=(8192,8192),timeout=750,
...
def _study(run_id):
    ...
    if live.get('pending') or live['next_at']-time.time()<800:return dict(status='waiting_for_training_gap')
    reservation=reserve(state/'budget.json',True,seconds=2250,startup_seconds=30,memory_gib=8,limit_override=85,authorized_monthly_limit=100)
    ...
    try:
        ...
        plan.update(parent=parent,head_checkpoint=dict(path=str(state/'solana-live'/parent/'head-final.pt'),sha256=digest(state/'solana-live'/parent/'head-final.pt')),
                    ...
                    synthetic_cadence_arms=[[1,5],[4,5],[4,2.5]],native_arm_seconds=180,
